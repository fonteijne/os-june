...
| Beta feedback (round 1) | **done** | A project can be given its Bonzai key as it is created; a Bonzai refusal in chat names its reason | `useBonzaiCreateKey` in the create dialog (probe first, store once the project has an id); `agent-runtime/src/bonzai.ts` + `bonzaiNoticePart` replace the "Clovy stopped unexpectedly" fallback for `bonzai_*` and `egress_blocked` codes |
...
| Beta feedback (round 1) | - | 26 | 145 |
| **Beta total** | **~37** | **145** | **145 / 150 (ADR-0060)** |
