...
- [adr/0057](adr/0057-bonzai-is-the-only-inference-egress.md) - fork-only, **superseded by ADR-0059**: Bonzai is the sole inference destination, enforced by a host allowlist at both HTTP chokepoints plus a CI egress test; inference egress is closed while MCP tool egress is governed (allowlisted streamable_http only, stdio disabled), and web search, image, video, and computer use are switched off rather than rerouted
- [adr/0058](adr/0058-bonzai-routing-lives-in-an-additive-provider-layer.md) - fork-only, **budget clause superseded by ADR-0060**: Bonzai routing is an additive provider layer - a new PROVIDER_BONZAI and `bonzai/` module, one three-line dispatch prologue per operation, a sub-40-line touched-line budget in shared files, an UPSTREAM.md ledger, and a CI conflict canary - extending the whitelabel additive-layer doctrine to provider dispatch
- [adr/0059](adr/0059-bonzai-egress-is-enforced-by-a-build-time-allowlist.md) - fork-only: supersedes ADR-0057 - the Bonzai host allowlist is a compile-time constant no runtime input can reach (a base URL is where we send, the allowlist is where we may send; deriving one from the other validates nothing), enforcement covers every reqwest client construction site in the native shell (sixteen across eight files, corrected from the eight this ADR first inventoried - see its 2026-09-05 addendum), and a source-level CI guard fails the build when an upstream merge introduces a new client
- [adr/0060](adr/0060-the-bonzai-touched-line-budget-is-a-shape-rule-with-an-inventoried-ceiling.md) - fork-only: supersedes ADR-0058's budget clause - every shared-file edit must be one of four shapes (prologue, substitution, flip, wrap), the beta ceiling is 150 counted lines with an exact ledger, and appended blocks are tracked but uncounted; records why the beta landed at 119 against an estimate of 37
- [adr/0061](adr/0061-local-byoi-transcription-reuses-provider-local.md) - fork-only: bring-your-own transcription (any OpenAI-compatible `/v1/audio/transcriptions` server, e.g. speaches) reuses `PROVIDER_LOCAL` under ADR-0058's test with its own `local_transcription` setting, `sanitize_settings` keeps the local provider across reloads, and the note-cleanup pass skips local transcripts (load-bearing for the privacy claim, previously masked by the dictation kill switch); no `bonzai/egress.rs` change needed
...
- [bonzai-implementation-plan.md](bonzai-implementation-plan.md) — fork-only: file-by-file plan for the Bonzai routing work across seven phases, with the additive `src-tauri/src/bonzai/` module layout, the dispatch-prologue shape, two-part egress enforcement (runtime check plus a source-level CI guard), per-phase verification, and the touched-line budget accounting
- [local-transcription-speaches-plan.md](local-transcription-speaches-plan.md) — fork-only, implemented (ADR-0061): BYO local speech-to-text server (reference backend: speaches) for fully local note transcription and dictation, reusing the existing `PROVIDER_LOCAL` BYOI pattern and `local_http_client()` (no `bonzai/egress.rs` change needed); companion/alternative to the separately-proposed bundled sherpa-onnx/Parakeet sidecar plan
