...
- [adr/0057](adr/0057-bonzai-is-the-only-inference-egress.md) - fork-only: Bonzai is the sole inference destination, enforced by a host allowlist at both HTTP chokepoints plus a CI egress test; inference egress is closed while MCP tool egress is governed (allowlisted streamable_http only, stdio disabled), and web search, image, video, and computer use are switched off rather than rerouted
