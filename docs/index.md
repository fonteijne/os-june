...
- [adr/0061](adr/0061-clovy-api-returns-as-bonzais-tee-hosted-single-hop.md) - fork-only: Clovy API returns as Bonzai's mandatory, TEE-hosted single hop (client → Clovy API → Bonzai, nothing else on either leg) now that the OS-Accounts-data-sharing reason for removing it was checked and does not hold; two-tier compiled egress (client-side allowlist narrows to Clovy API, a new Clovy-API-side allowlist narrows to Bonzai); Venice BYOK is removed entirely, not carved out as an exception
- [adr/0062](adr/0062-clovy-apis-non-inference-surfaces-stay-disabled-pending-their-dependencies.md) - fork-only: issue reporting, private sharing, and JWKS-based auth in Clovy API stay disabled (not rejected) until their real dependencies exist - a Jira sink, a redesigned share-viewer without an OS Accounts login, and Entra ID respectively
