...
- [adr/0058](adr/0058-bonzai-routing-lives-in-an-additive-provider-layer.md) - fork-only: Bonzai routing is an additive provider layer - a new PROVIDER_BONZAI and `bonzai/` module, one three-line dispatch prologue per operation, a sub-40-line touched-line budget in shared files, an UPSTREAM.md ledger, and a CI conflict canary - extending the whitelabel additive-layer doctrine to provider dispatch
