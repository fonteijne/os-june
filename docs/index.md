...
- [adr/0056](adr/0056-whitelabel-branding-as-additive-config-layer.md) - whitelabel branding lives in an additive `branding/` config/asset layer with build-time composition, never in-place rebrand edits, to keep upstream merges conflict-free; orthogonal to ADR-0054/0055's June-era compatibility bridge
...
- [whitelabel-implementation-plan.md](whitelabel-implementation-plan.md) — plan to enable whitelabel branding (app identity, icons, UI copy, backend copy, per-brand OS Accounts/updater identity) while keeping fork updates from upstream `os-june` conflict-free; Phases 1-5 implemented; see [ADR-0056](adr/0056-whitelabel-branding-as-additive-config-layer.md)
- [whitelabel-release-runbook.md](whitelabel-release-runbook.md) — Phase 4 checklist: what a real whitelabel partner needs once (OS Accounts OAuth client, releases repo, signing identities, keychain service id) before it can build, sign, and auto-update
