...
- [adr/0059](adr/0059-bonzai-egress-is-enforced-by-a-build-time-allowlist.md) - fork-only: supersedes ADR-0057 - the Bonzai host allowlist is a compile-time constant no runtime input can reach (a base URL is where we send, the allowlist is where we may send; deriving one from the other validates nothing), enforcement covers all eight reqwest client construction sites across clovy_api.rs and agent_mcp.rs, and a source-level CI guard fails the build when an upstream merge introduces a new client
