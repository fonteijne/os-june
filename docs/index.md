...
- [local-transcription-parakeet-sidecar-plan.md](local-transcription-parakeet-sidecar-plan.md) — fork-only, proposed: bundle a sherpa-onnx + NVIDIA Parakeet ASR engine as a Clovy-managed native sidecar for fully local note transcription and dictation (no external server, no Clovy API/Bonzai egress); new `PROVIDER_ON_DEVICE` identity, download-on-enable model weights, CPU-only v1, and the `bonzai/egress.rs` allowlist addition the model download requires under ADR-0059
